 ...
 */
/*
不知道为何，用sio_put总是无法通过测试；只好使用printf
*/
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
 ...
 */
/*
实现内置命令kill,提取PID/JID,不必判断“ No such process group”
if the PID or JID is a negative argument,p=-p;
then use the kill function to send SIGTERM to the group
*/
void builtin_kill(struct cmdline_tokens tok)
{
    ...
    if(tok.argv[1][0]=='%')
    {
        char *number=tok.argv[1];
        memmove(number,number+1,strlen(number));
        p=atoi(number);
        if(p<0)
            p=-p;
        struct job_t *job= getjobjid(job_list,p);
        if(job)
        ...
        if(p<0)
           p=-p;
        struct job_t *job= getjobpid(job_list,p);
        if(job)
            kill(job->pid,SIGTERM);
        else
            printf("(%d):No such job\n",p);
...
/*
将作业放在后台运行,发送SIGCONT信号
bg内置命令也需要打印相关信息
*/
void bg_(struct cmdline_tokens tok)
{
    int p;
    if(tok.argv[1][0]=='%')
    {
...
/*
fg内置命令
前台作业只能有一个且必须等待他结束以后才能输入下一条命令
因为这里仍要像BUILTIN_NONE 的操作一样,让shell进程等待子进程的结束
*/
void fg_(struct cmdline_tokens tok)
{
    int p;
    if(tok.argv[1][0]=='%')
    {
    ...
    else
    {
        ...
        struct job_t *job = getjobpid(job_list, p);
        if (job) {
            job->state = FG;  // 更新状态为FG
        }
        kill(job->pid,SIGCONT);
    }
    /*
    父进程要等待子进程结束
    根据writeup和教材,这里应该使用sigsuspend
    */
    sigset_t prev;
    sigemptyset(&prev);
...
eval(char *cmdline) 
{
    ...
    if (tok.argv[0] == NULL) /* ignore empty lines */
        return;
    int output_fd=1;
    int input_fd=0;//标准输入输出
    int fd1,fd2;
    int saved_stdin = dup(input_fd);
    int saved_stdout = dup(output_fd);
    if(tok.infile!=NULL)
    {
    ...
    if(strcmp(tok.argv[0],"NEXT")==0||strcmp(tok.argv[0],"WAIT")==0||strcmp(tok.argv[0],"SIGNAL")==0)
       return;
    ...
    if(tok.builtins==BUILTIN_NONE){
        /*
        阻塞lab中要求的三种信号,由于父子进程运行顺序无法确定,
        阻塞SIGCHLD可以避免addjob和deletejob的竞争
        */
        sigset_t mask,prev;
        sigemptyset(mask);
        sigemptyset(prev);
        ...
        else if(pid>0)
        {
            ...
            else
            {
                ...
                sigprocmask(SIG_SETMASK,&prev,NULL);//恢复
                while(fgpid(job_list)!=0)//父进程要等待子进程结束
                    sigsuspend(&prev);
            }
        }
    }
    //因为后续还有新的输入输出,所以应该恢复重定向
    if(tok.infile!=NULL)
        dup2(saved_stdin, input_fd);
    if(tok.outfile!=NULL)
        dup2(saved_stdout, output_fd);
    return;
}
...
sigchld_handler(int sig) 
{
    ...
    while((pid=waitpid(-1,&status,WNOHANG|WUNTRACED))>0)
    {
        ...
        /*
        三种状态,子进程正常exit or return;子进程被信号终止；子进程被信号停止；
        终止和正常结束都应该deletejob
        */
        if(WIFEXITED(status))
        {
