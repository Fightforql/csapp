...
//修改方式：使用了pipe_std,arch cost是4
//原因：pipe_std通过流水线将指令分为多个阶段,避免了单一指令路径中串行等待的情况，关键路径更短,硬件资源利用率高
//通过尝试，发现在不更改pipe_std的情况下使用pipe_s4c的cpe最高;
//而后经过多次尝试，将pipe_std中cond和reg_cc的计算并行，这样可以的原因是对于需要改变cc的opq和iopq指令
//它们不需要读取cc参与cond的计算
...
crate::define_stages! {
    ...
    ExecuteStage e {
        stat: Stat = Bub, icode: u8 = NOP, ifun: u8 = 0,cc: ConditionCode = CC_INIT,
        valC: u64 = 0,
