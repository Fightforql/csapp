 ...
 *   Rating: 4
 */
int countTrailingZero(unsigned x) {
  int res = (!(x&0x0000FFFF))<<4;
	res+=(!(x&(0x000000FF<<res)))<<3;
	res+=(!(x&(0x0000000F<<res)))<<2;
	res+=(!(x&(0x00000003<<res)))<<1;
	res+=(!(x&(0x00000001<<res)));
	res+=(!(x&(0x00000001<<res)));
  return res;
}
...
int isLessOrEqual(int x, int y) {
  int flag=(x>>31)^(y>>31);
  int res1=((x>>31)&1)&(~(y>>31)&1);
  int res2=~((y+(~x+1))>>31)&1;
  return (flag&res1)|(~flag&res2);
}
/*
 ...
 *   they are to be intSerpreted as the bit-level representation of
